...
class Temporal_feature_EEG(nn.Module):
    def __init__(self,inplanes):
        ...
        self.features1 = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=50, stride=8, bias=False, padding=24),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(), 
        ...
        self.features2 = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=500, stride=64, bias=False, padding=24),
            nn.BatchNorm1d(64),
            nn.LeakyReLU(), 
...
class MMASleepNet_EEG(BasicModel):
    def __init__(self,config):#,d_model,afr_reduced_cnn_size):
        super(MMASleepNet_EEG, self).__init__()
        
        self.afr_reduced_cnn_size=config.afr_reduced_cnn_size
        self.d_model = config.d_model
        self.inplanes = config.inplanes
        self.nhead=config.nhead
        self.num_layers=config.num_layers
        self.features_len=config.features_len

        self.EEG_feature = Temporal_feature_EEG(self.inplanes)
        self.linear1 = nn.Sequential(nn.Linear(self.d_model*self.afr_reduced_cnn_size*32,self.features_len),nn.ReLU(True))
        self.linear2 = nn.Sequential(nn.Linear(self.features_len,5),nn.Softmax(dim=1))
        #init params way
    ...
    def forward(self, x1):#,x2,x3):
        ...
        x_afr = self.AFR(x_EEG) #[256,2,64,24]
        # print(x_cat.shape)
        x_afr= x_afr.view(self.batch_size,-1,48) 
        # x_cat=x_cat.squeeze()
        # print(x_afr.shape)
        encoded_features=self.transformer_encoder(x_afr.view(self.batch_size,-1,48) )
        ...
        logits = self.linear2(x_final)
        # x_final = self.linear2(x_final)
        # print('x_final.shape',x_final.shape)
        return logits, (x_final, encoded_features)
